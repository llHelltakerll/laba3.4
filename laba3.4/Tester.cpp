...
void Tester::doTask(Product& product) {
	string task_noties;
	//cin.ignore(10000, '\n');
	cout << "Enter task noties: ";
	getline(cin, task_noties);
	product.setProductTaskNoties(task_noties);
}

void Tester::chooseTask(vector<Product>& product_arr) {
	int comd;
	int prod_id{};
	int val_prod_el{};
	int time;
